...
@Tag(name = "签订合作企业记录管理")
public class CooperativeEnterpriseRecordController {

    @Resource
    private CooperativeEnterpriseRecordService recordService;
    @Resource
    private UserService userService;

    @PostMapping("/add")
    @Operation(summary = "提交签订合作企业记录")
    ...
    public BaseResponse<Page<CooperativeEnterpriseRecordVO>> listMyRecords(
            @RequestBody CooperativeEnterpriseQueryRequest queryRequest,
            HttpServletRequest httpRequest) {
    ...
    public BaseResponse<CooperativeEnterpriseRecordVO> getRecordById(@PathVariable Long id) {
        ThrowUtils.throwIf(id == null || id <= 0, ErrorCode.PARAMS_ERROR);
        CooperativeEnterpriseRecord record = recordService.getById(id);
        CooperativeEnterpriseRecordVO vo = recordService.getRecordVO(record);
    ...
    public BaseResponse<Boolean> deleteRecord(@RequestBody DeleteRequest deleteRequest,
        ...
        User loginUser = userService.getLoginUser(httpRequest);
        CooperativeEnterpriseRecord record = recordService.getById(deleteRequest.getId());
    ...
    public BaseResponse<Page<CooperativeEnterpriseRecordVO>> adminListRecords(
            @RequestBody CooperativeEnterpriseQueryRequest queryRequest) {
        ThrowUtils.throwIf(queryRequest == null, ErrorCode.PARAMS_ERROR);
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    @Operation(summary = "导出签订合作企业Excel")
    public void exportRecords(HttpServletResponse response) {
        ...
        response.setHeader("Content-Disposition",
                "attachment; filename=cooperative_enterprise.xlsx");
