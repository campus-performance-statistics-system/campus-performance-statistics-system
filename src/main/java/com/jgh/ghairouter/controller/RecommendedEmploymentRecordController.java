...
@RestController
@RequestMapping("/recommended-employment")
@Tag(name = "推荐学院学生签约就业记录管理")
public class RecommendedEmploymentRecordController {

    @Resource
    private RecommendedEmploymentRecordService recordService;
    @Resource
    private UserService userService;

    @PostMapping("/add")
    @Operation(summary = "提交推荐学院学生签约就业记录")
    public BaseResponse<Long> addRecord(
            @RequestParam("teacherName") String teacherName,
            @RequestParam("companyName") String companyName,
            @RequestParam(value = "contractCount", required = false) Integer contractCount,
            @RequestParam(value = "recommendationTime", required = false) String recommendationTime,
    ...
    public BaseResponse<Page<RecommendedEmploymentRecordVO>> listMyRecords(
            @RequestBody RecommendedEmploymentQueryRequest queryRequest,
            HttpServletRequest httpRequest) {
    ...
    public BaseResponse<RecommendedEmploymentRecordVO> getRecordById(@PathVariable Long id) {
        ThrowUtils.throwIf(id == null || id <= 0, ErrorCode.PARAMS_ERROR);
        RecommendedEmploymentRecord record = recordService.getById(id);
        RecommendedEmploymentRecordVO vo = recordService.getRecordVO(record);
    ...
    public BaseResponse<Boolean> deleteRecord(@RequestBody DeleteRequest deleteRequest,
        ...
        User loginUser = userService.getLoginUser(httpRequest);
        RecommendedEmploymentRecord record = recordService.getById(deleteRequest.getId());
    ...
    public BaseResponse<Page<RecommendedEmploymentRecordVO>> adminListRecords(
            @RequestBody RecommendedEmploymentQueryRequest queryRequest) {
        ThrowUtils.throwIf(queryRequest == null, ErrorCode.PARAMS_ERROR);
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    @Operation(summary = "导出推荐学院学生签约就业Excel")
    public void exportRecords(HttpServletResponse response) {
        ...
        response.setHeader("Content-Disposition",
                "attachment; filename=recommended_employment.xlsx");
