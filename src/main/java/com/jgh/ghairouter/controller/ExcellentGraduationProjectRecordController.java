...
@RestController
@RequestMapping("/excellent-graduation-project")
@Tag(name = "优秀毕设记录管理")
public class ExcellentGraduationProjectRecordController {

    @Resource
    private ExcellentGraduationProjectRecordService recordService;
    @Resource
    private UserService userService;

    @PostMapping("/add")
    @Operation(summary = "提交优秀毕设记录")
    ...
    public BaseResponse<Page<ExcellentGraduationProjectRecordVO>> listMyRecords(
            @RequestBody ExcellentGraduationProjectQueryRequest queryRequest,
            HttpServletRequest httpRequest) {
    ...
    public BaseResponse<ExcellentGraduationProjectRecordVO> getRecordById(@PathVariable Long id) {
        ThrowUtils.throwIf(id == null || id <= 0, ErrorCode.PARAMS_ERROR);
        ExcellentGraduationProjectRecord record = recordService.getById(id);
        ExcellentGraduationProjectRecordVO vo = recordService.getRecordVO(record);
    ...
    public BaseResponse<Boolean> deleteRecord(@RequestBody DeleteRequest deleteRequest,
        ...
        User loginUser = userService.getLoginUser(httpRequest);
        ExcellentGraduationProjectRecord record = recordService.getById(deleteRequest.getId());
    ...
    public BaseResponse<Page<ExcellentGraduationProjectRecordVO>> adminListRecords(
            @RequestBody ExcellentGraduationProjectQueryRequest queryRequest) {
        ThrowUtils.throwIf(queryRequest == null, ErrorCode.PARAMS_ERROR);
    ...
    public void exportRecords(HttpServletResponse response) {
        ...
        response.setHeader("Content-Disposition",
                "attachment; filename=excellent_graduation_project.xlsx");
