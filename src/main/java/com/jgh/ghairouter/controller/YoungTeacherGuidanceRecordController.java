...
@RestController
@RequestMapping("/young-teacher-guidance")
@Tag(name = "指导青年教师记录管理")
public class YoungTeacherGuidanceRecordController {

    @Resource
    private YoungTeacherGuidanceRecordService recordService;
    @Resource
    private UserService userService;

    @PostMapping("/add")
    @Operation(summary = "提交指导青年教师记录")
    public BaseResponse<Long> addRecord(
            @RequestParam("college") String college,
            @RequestParam("mentorNames") String mentorNames,
            @RequestParam("mentorTitles") String mentorTitles,
            @RequestParam("youngTeacherName") String youngTeacherName,
            @RequestParam("youngTeacherEntryTime") String youngTeacherEntryTime,
    ...
    public BaseResponse<Page<YoungTeacherGuidanceRecordVO>> listMyRecords(
            @RequestBody YoungTeacherGuidanceQueryRequest queryRequest,
            HttpServletRequest httpRequest) {
    ...
    public BaseResponse<YoungTeacherGuidanceRecordVO> getRecordById(@PathVariable Long id) {
        ThrowUtils.throwIf(id == null || id <= 0, ErrorCode.PARAMS_ERROR);
        YoungTeacherGuidanceRecord record = recordService.getById(id);
        YoungTeacherGuidanceRecordVO vo = recordService.getRecordVO(record);
    ...
    public BaseResponse<Boolean> deleteRecord(@RequestBody DeleteRequest deleteRequest,
        ...
        User loginUser = userService.getLoginUser(httpRequest);
        YoungTeacherGuidanceRecord record = recordService.getById(deleteRequest.getId());
    ...
    public BaseResponse<Page<YoungTeacherGuidanceRecordVO>> adminListRecords(
            @RequestBody YoungTeacherGuidanceQueryRequest queryRequest) {
        ThrowUtils.throwIf(queryRequest == null, ErrorCode.PARAMS_ERROR);
    ...
    @AuthCheck(mustRole = UserConstant.ADMIN_ROLE)
    @Operation(summary = "导出指导青年教师Excel")
    public void exportRecords(HttpServletResponse response) {
        ...
        response.setHeader("Content-Disposition",
                "attachment; filename=young_teacher_guidance.xlsx");
