...
public class StatisticsServiceImpl implements StatisticsService {
    ...
    private static final String ALL_SCORE_SQL = """
            SELECT
              u.id AS user_id,
              u.user_name,
              COALESCE(tcs.teacher_score, 0) + COALESCE(ascore.student_score, 0) + COALESCE(tscore.training_score, 0) + COALESCE(rscore.research_score, 0) + COALESCE(iscore.innovation_score, 0) AS total_score
            FROM user u
            LEFT JOIN (
              SELECT
                user_id,
            ...
            WHERE u.is_delete = 0
              AND (tcs.teacher_score IS NOT NULL OR ascore.student_score IS NOT NULL OR tscore.training_score IS NOT NULL OR rscore.research_score IS NOT NULL OR iscore.innovation_score IS NOT NULL)
            """;
