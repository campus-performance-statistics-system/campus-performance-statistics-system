...
import com.jgh.ghairouter.model.constants.YoungTeacherGuidanceScoringConstants;
...
import com.jgh.ghairouter.service.YoungTeacherGuidanceRecordService;
...
@Slf4j
@Service
public class YoungTeacherGuidanceRecordServiceImpl
        extends ServiceImpl<YoungTeacherGuidanceRecordMapper, YoungTeacherGuidanceRecord>
        implements YoungTeacherGuidanceRecordService {
    ...
    public Long addRecord(Long userId,
                          ...
                          String mentorNames,
                          String mentorTitles,
                          String youngTeacherName,
                          String youngTeacherEntryTime,
                          MultipartFile file) {
        ...
        YoungTeacherGuidanceRecord record = new YoungTeacherGuidanceRecord();
        record.setUserId(userId);
        record.setTypeName(DEFAULT_TYPE_NAME);
        record.setCollege(college);
        record.setMentorNames(mentorNames);
        record.setMentorTitles(mentorTitles);
        record.setYoungTeacherName(youngTeacherName);
        record.setYoungTeacherEntryTime(youngTeacherEntryTime);
    ...
    @Override
    public QueryWrapper getQueryWrapper(YoungTeacherGuidanceQueryRequest req) {
        ...
        QueryWrapper wrapper = QueryWrapper.create()
                ...
                .like("mentor_names", req.getMentorNames())
                .like("young_teacher_name", req.getYoungTeacherName());
    ...
    @Override
    public YoungTeacherGuidanceRecordVO getRecordVO(YoungTeacherGuidanceRecord record) {
        if (record == null) return null;
        YoungTeacherGuidanceRecordVO vo = new YoungTeacherGuidanceRecordVO();
    ...
    @Override
    public Page<YoungTeacherGuidanceRecordVO> pageRecords(YoungTeacherGuidanceQueryRequest req) {
        long pageNum = req.getPageNum();
        long pageSize = req.getPageSize();

        QueryWrapper wrapper = getQueryWrapper(req);
        if (StrUtil.isNotBlank(req.getAdminReviewStatus())) {
            Page<YoungTeacherGuidanceRecord> recordPage = this.page(Page.of(pageNum, pageSize), wrapper);
            List<YoungTeacherGuidanceRecordVO> voList = recordPage.getRecords().stream()
                    .map(this::getRecordVO)
                    .filter(vo -> req.getAdminReviewStatus().equals(vo.getAdminReviewStatus()))
                    .collect(Collectors.toList());
            Page<YoungTeacherGuidanceRecordVO> voPage = new Page<>(pageNum, pageSize, recordPage.getTotalRow());
    ...
    @Override
    public Page<YoungTeacherGuidanceRecordVO> pageMyRelatedRecords(Long userId, YoungTeacherGuidanceQueryRequest req) {
        long pageNum = req.getPageNum();
        long pageSize = req.getPageSize();
    ...
    public void adminReview(Long recordId, String reviewStatus, String reviewComment, Long adminId) {
        ...
        YoungTeacherGuidanceRecord record = this.getById(recordId);
    ...
    public byte[] exportRecordsToExcel() {
        try (XSSFWorkbook workbook = new XSSFWorkbook()) {
            ...
            Sheet sheet = workbook.createSheet("14-指导青年教师");

            // 列宽
            sheet.setColumnWidth(0, (int) (8.22 * 256));   // A: 序号
            sheet.setColumnWidth(1, (int) (18.22 * 256));  // B: 教学单位
            sheet.setColumnWidth(2, (int) (22.00 * 256));  // C: 指导教师-姓名
            sheet.setColumnWidth(3, (int) (22.00 * 256));  // D: 指导教师-职称
            sheet.setColumnWidth(4, (int) (16.00 * 256));  // E: 青年教师-姓名
            sheet.setColumnWidth(5, (int) (18.00 * 256));  // F: 青年教师-入职时间

            // 获取所有记录，按学院、创建时间排序
            List<YoungTeacherGuidanceRecord> allRecords = this.list(
                    QueryWrapper.create().orderBy("college", true)
                            .orderBy("create_time", true));

            // 按学院分组（保持插入顺序）
            Map<String, List<YoungTeacherGuidanceRecord>> collegeGroups = new LinkedHashMap<>();
            for (YoungTeacherGuidanceRecord r : allRecords) {
            ...
            titleCell.setCellValue(year + "年指导青年教师汇总");
            ...
            String[] headers = {"序号", "教学单位", "指导教师-姓名", "指导教师-职称", "青年教师-姓名", "青年教师-入职时间"};
            ...
            int seq = 1;
            for (Map.Entry<String, List<YoungTeacherGuidanceRecord>> entry : collegeGroups.entrySet()) {
                List<YoungTeacherGuidanceRecord> records = entry.getValue();

                for (int i = 0; i < records.size(); i++) {
                    YoungTeacherGuidanceRecord r = records.get(i);
                    ...
                    cellC.setCellValue(r.getMentorNames() != null ? r.getMentorNames() : "");
                    cellC.setCellStyle(dataStyle);

                    // D: 指导教师-职称
                    Cell cellD = dataRow.createCell(3);
                    cellD.setCellValue(r.getMentorTitles() != null ? r.getMentorTitles() : "");
                    cellD.setCellStyle(dataStyle);

                    // E: 青年教师-姓名
                    Cell cellE = dataRow.createCell(4);
                    cellE.setCellValue(r.getYoungTeacherName() != null ? r.getYoungTeacherName() : "");
                    cellE.setCellStyle(dataStyle);

                    // F: 青年教师-入职时间
                    Cell cellF = dataRow.createCell(5);
                    cellF.setCellValue(r.getYoungTeacherEntryTime() != null ? r.getYoungTeacherEntryTime() : "");
