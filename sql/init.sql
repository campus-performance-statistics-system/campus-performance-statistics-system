...
-- ===================== 已有数据库迁移：移除旧的外键级联删除和触发器 =====================
-- 注意：删除操作已改为代码层面的软删除（is_delete=1），不再依赖MySQL外键级联和触发器
-- 如果数据库已存在旧的级联外键和触发器，请执行以下语句清理：

-- 【移除旧的外键级联删除约束】
-- 说明：MySQL 的 ALTER TABLE DROP FOREIGN KEY 不支持 IF EXISTS 语法，
-- 可通过查询 information_schema 判断外键是否存在后执行删除：
SET @fk_exists = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'user_competition_score'
    AND CONSTRAINT_NAME = 'fk_user_competition_score_record');
SET @sql = IF(@fk_exists > 0, 'ALTER TABLE user_competition_score DROP FOREIGN KEY fk_user_competition_score_record', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @fk_exists = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'advisor_score'
    AND CONSTRAINT_NAME = 'fk_advisor_score_record');
SET @sql = IF(@fk_exists > 0, 'ALTER TABLE advisor_score DROP FOREIGN KEY fk_advisor_score_record', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @fk_exists = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'training_guidance_score'
    AND CONSTRAINT_NAME = 'fk_training_guidance_score_record');
SET @sql = IF(@fk_exists > 0, 'ALTER TABLE training_guidance_score DROP FOREIGN KEY fk_training_guidance_score_record', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @fk_exists = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'research_achievement_score'
    AND CONSTRAINT_NAME = 'fk_research_achievement_score_record');
SET @sql = IF(@fk_exists > 0, 'ALTER TABLE research_achievement_score DROP FOREIGN KEY fk_research_achievement_score_record', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @fk_exists = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'innovation_entrepreneurship_score'
    AND CONSTRAINT_NAME = 'fk_innovation_entrepreneurship_score_record');
SET @sql = IF(@fk_exists > 0, 'ALTER TABLE innovation_entrepreneurship_score DROP FOREIGN KEY fk_innovation_entrepreneurship_score_record', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @fk_exists = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'teaching_reform_score'
    AND CONSTRAINT_NAME = 'fk_teaching_reform_score_record');
SET @sql = IF(@fk_exists > 0, 'ALTER TABLE teaching_reform_score DROP FOREIGN KEY fk_teaching_reform_score_record', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @fk_exists = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'thesis_score'
    AND CONSTRAINT_NAME = 'fk_thesis_score_record');
SET @sql = IF(@fk_exists > 0, 'ALTER TABLE thesis_score DROP FOREIGN KEY fk_thesis_score_record', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @fk_exists = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'sports_event_score'
    AND CONSTRAINT_NAME = 'fk_sports_event_score_record');
SET @sql = IF(@fk_exists > 0, 'ALTER TABLE sports_event_score DROP FOREIGN KEY fk_sports_event_score_record', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;

SET @fk_exists = (SELECT COUNT(*) FROM information_schema.TABLE_CONSTRAINTS
    WHERE CONSTRAINT_SCHEMA = DATABASE() AND TABLE_NAME = 'part_time_class_advisor_score'
    AND CONSTRAINT_NAME = 'fk_part_time_class_advisor_score_record');
SET @sql = IF(@fk_exists > 0, 'ALTER TABLE part_time_class_advisor_score DROP FOREIGN KEY fk_part_time_class_advisor_score_record', 'SELECT 1');
PREPARE stmt FROM @sql; EXECUTE stmt; DEALLOCATE PREPARE stmt;
